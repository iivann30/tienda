...
class envioEmail { 
    ...
    function sendMail() {
        ...
        $mail->Username = 'tiendaasir960@gmail.com';

        //Contraseña de gmail para la SMTP authentication
        $mail->Password = 'frodobolson';

        //Asignar el 'desde'
        $mail->setFrom('tiendaasir960@gmail.com', 'Tienda asir el mejor grupo');

        // reply-to address
        $mail->addReplyTo('replyto@example.com', 'First Last');

        //Dirección de envío
        $mail->addAddress('$this->email', '$this->name $this->apellidos');

        //Ponemos el asunto
        $mail->Subject = 'Cliente registrado';

        //Read an HTML message body from an external file, convert referenced images to embedded,
        //convert HTML into a basic plain-text alternative body
        //$mail->msgHTML(file_get_contents('contents.html'), __DIR__);
        $mail->msgHTML("<p>Bienvenido a la tienda. Gracias por registrarse.</p>", __DIR__);
        //Replace the plain text body with one created manually
        $mail->AltBody = 'Tienda asir';
